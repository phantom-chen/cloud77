...
namespace SuperService.Controllers
{
    ...
    public class DatabaseController : ControllerBase
    {
        ...
        // get email token or password token for testing purpose
        [HttpGet()]
        [Route("tokens")]
        public async Task<IActionResult> GetTokens([FromQuery] string email)
        {
            var database = client.GetDatabase(configuration["Database"]);
            var events = new EventCollection(database);
            var logs = events.GetEventLogs(email, "");
            
            return Ok(logs.Where(l => l.Name == "Email-Token" || l.Name == "Password-Token"));
        }
