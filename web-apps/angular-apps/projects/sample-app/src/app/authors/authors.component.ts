...
export class AuthorsComponent implements AfterViewInit {
  ...
  getAuthors(): void {
    this.http.get<AuthorResult>(`/sample-api/authors?index=0&size=10`)
      .subscribe({
  ...
  openDialog(action: 'create' | 'update', author: AuthorEntity) {
    ...
    dialogRef.afterClosed().subscribe((res: { action: string, data: AuthorEntity }) => {
      if (res) {
        switch (res.action) {
          case 'create':
            this.http.post<DefaultResponse>('/sample-api/authors', res.data)
              .subscribe({
            ...
            break;
          case 'update':
            this.http.put<DefaultResponse>(`/sample-api/authors/${res.data.id}`, res.data)
              .subscribe({
                next: (res: DefaultResponse) => {
            ...
            break;
          case 'delete':
            this.http.delete<DefaultResponse>(`/sample-api/authors/${res.data.id}`)
              .subscribe({
                next: (res: DefaultResponse) => {
